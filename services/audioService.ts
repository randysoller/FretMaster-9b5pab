...
class AudioService {
    ...
    stringsToPlay.forEach(({ stringIndex, frequency }, arrayIndex) => {
      ...
      for (let i = startSample; i < durationSamples; i++) {
        ...
        if (t < 0.001) {
          ...
          // Sustain + Release (exponential decay to zero)
          const sustainLevel = baseVolume * 0.75;
          const decayFactor = (t - 0.08) / (duration * 0.2); // Aggressive decay
          envelope = sustainLevel * Math.exp(-decayFactor * 5); // Reaches near-zero by end
        }
  ...
  private async playChordNative(chord: ChordData): Promise<void> {
    ...
    try {
      ...
      const wavDataUri = this.generateGuitarWAV(chord, 3.3);
  ...
  async playChordPreview(chordInput: ChordData | string): Promise<void> {
    ...
    try {
      ...
      if (Platform.OS === 'ios' || Platform.OS === 'android') {
        ...
        await this.playChordSynthesis(chordInput, 3300);
      }
